              ...
              PageView.builder(
                ...
                itemBuilder: (context, index) {
                  return GestureDetector(
                    onTap: () => Navigator.push(
                      ...
                      PageRouteBuilder(
                        ...
                        pageBuilder: (BuildContext context, _, __) => FullScreenGallery(
                          title: widget.model.author?.username ?? '',
                    ...
                    child: Hero(
                      tag: widget.model.gallery?[index].id.toString() ?? index,
        ...
        return IntrinsicHeight(
          child: Stack(
            children: [
                        ...
                        children: [
                          ...
                          PopupMenuButton(
                            ...
                            itemBuilder: ((context) => [
                              ...
                              if(widget.model.isPinned == true) BoardTypePopupMenuItem(
                                title: 'Открепить',
                                image: Image.asset(
                                ...
                                onTap: () async {
                                  ...
                                  await messageDialogsProvider.theSelectionWindow(
                                    context: context,
                                    title: 'Вы уверены, что хотите открепить пост?',
                                    yesButton: 'ДА',
                                    noButton: 'Я ПЕРЕДУМАЛ',
                                    yesOnTap: () async {
                                      setState(() => widget.model.isLoading = true);
                                      Navigator.pop(tabBarProvider.mainContext);
                                      await profileCreationProvider.unPinPost(
                                        widget.model.id ?? 0,
                                        ((model) async {
                                          if(model?.status == true) {
                                            await catalogProvider.updatePostsOfCommunity(catalogProvider.communityProfileModel?.id ?? 0).whenComplete(
                                              (() => setState(() => widget.model.isLoading = false)),
                                            );
                                          } else {
                                            setState(() => widget.model.isLoading = false);
                              ...
                              if(widget.model.isPinned == false) BoardTypePopupMenuItem(
                                title: 'Закрепить',
                                image: Image.asset(
                                  ConstantsAssets.pinPostImage,
                                  height: 1.65.h,
                                ...
                                onTap: () async {
                                  ...
                                  await messageDialogsProvider.theSelectionWindow(
                                    ...
                                    yesOnTap: () async {
                                      setState(() => widget.model.isLoading = true);
                                      Navigator.pop(tabBarProvider.mainContext);
                                      await profileCreationProvider.pinPost(
                                        widget.model.id ?? 0,
                                        ((model) async {
                                          if(model?.status == true) {
                                            await catalogProvider.updatePostsOfCommunity(catalogProvider.communityProfileModel?.id ?? 0).whenComplete(
                                ...
                                onTap: () async {
                                  ...
                                  await messageDialogsProvider.theSelectionWindow(
                                    ...
                                    yesOnTap: () async {
                                      Navigator.pop(tabBarProvider.mainContext);
                                      setState(() => widget.model.isLoading = true);
                                      await profileCreationProvider.deletePost(
                                        widget.model.id ?? 0,
                                        ((model) async {
                                          if(model?.status == true) {
                                            await catalogProvider.updatePostsOfCommunity(catalogProvider.communityProfileModel?.id ?? 0).whenComplete(
              ...
              if(widget.model.isLoading == true) ClipRRect(
